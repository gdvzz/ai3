...
def dj_json_to_markdown(
    input_file: str = "longfor_jobs_dj_dedup.json",
    output_file: str = "longfor_jobs_dj_final.md",
):
    """
    将'点将'项目去重后的JSON转换为Markdown文档
    """
    ...
    print(f"读取到 {len(jobs)} 个'点将'岗位")

    # 生成Markdown
    md_lines = []

    # 添加标题
    md_lines.append("# 龙湖'点将'校招岗位汇总\n")
    ...
    print(f"共生成 {len(jobs)} 个'点将'岗位条目")


def main():
    # 转换去重后的JSON为Markdown
    dj_json_to_markdown(
        input_file="longfor_jobs_dj_dedup.json", output_file="longfor_jobs_dj_final.md"
    )
