...
def deduplicate_and_merge_zf_jobs(
    input_file: str = "longfor_jobs_zf.json",
    output_file: str = "longfor_jobs_zf_dedup.json",
):
    """
    专门处理'绽放'项目岗位数据：
    ...
    print(f"去重后'绽放'岗位数: {len(dedup_jobs)}")
...
def generate_zf_markdown_report(
    dedup_jobs: list, output_file: str = "longfor_jobs_zf_report.md"
):
    """生成'绽放'项目的Markdown报告"""

    with open(output_file, "w", encoding="utf-8") as f:
        f.write("# 龙湖'绽放'校招岗位汇总\n\n")
        f.write(
...
def main():
    # 1. 去重处理
    dedup_jobs = deduplicate_and_merge_zf_jobs(
        input_file="longfor_jobs_zf.json", output_file="longfor_jobs_zf_dedup.json"
    )

    # 2. 生成Markdown报告
    generate_zf_markdown_report(
        dedup_jobs=dedup_jobs, output_file="longfor_jobs_zf_report.md"
    )
