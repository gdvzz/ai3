...
class LongForJobCrawler:
    def __init__(self, output_file: str = "longfor_jobs_dj.json"):
    ...
    async def get_sub_companies(self, page) -> List[Dict[str, Any]]:
        ...
        try:
            # 访问"点将"项目页面建立会话
            await page.goto(
                "https://longfor.zhaopin.com/dj/index.html",
            ...
            payload = {
                ...
                "companyOrDepartmentId": "10126973",  # "点将"项目的ID
                "format": "tree",
            }
    ...
    def _get_fallback_companies(self):
        """获取备用的公司列表（基于"点将"项目API返回的数据）"""
        fallback = [
            {"id": 10126994, "name": "龙湖智创生活", "level": "parent"},
        ]
    ...
    async def process_company(self, page, company: Dict[str, Any]):
        ...
        while page_index <= total_pages:
            # 构建请求参数 - jobSource可能需要根据"点将"项目调整
            params = {
                "orgNumbers": "105173",
                "jobSource": 2,  # 假设"点将"项目使用相同的jobSource值
    ...
    async def crawl(self):
        ...
        logger.info("=" * 60)
        logger.info("开始爬取龙湖'点将'校招岗位信息")
        logger.info(f"目标页面: https://longfor.zhaopin.com/dj/index.html")
...
async def main():
    crawler = LongForJobCrawler("longfor_jobs_dj.json")
    await crawler.crawl()
